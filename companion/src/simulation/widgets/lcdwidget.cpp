...
void LcdWidget::setBackgroundColor(const QColor & color)
{
  bgColor = color;
}

void LcdWidget::setAcceptTouchEvents(bool on)
{
  acceptTouchEvents = on;
  setAttribute(Qt::WA_AcceptTouchEvents, on);
}
...
bool LcdWidget::touchEvent(QTouchEvent * event)
{
  if (!event)
    return false;

//  QTouchEvent tmp(event->type(), NULL, event->modifiers(), event->touchPointStates());
//  QList<QTouchEvent::TouchPoint> ptlist;
//  foreach (const QTouchEvent::TouchPoint & tp, event->touchPoints()) {
//    QTouchEvent::TouchPoint tpcpy(tp);
//    ptlist.append(tpcpy);
//  }
//  tmp.setTouchPoints(ptlist);
  //emit simulatorTouchEvent(event);
  event->accept();
  return true;
}
...
bool LcdWidget::event(QEvent * event)
{
  ...
  if (acceptTouchEvents && (event->type() == QEvent::TouchBegin || event->type() == QEvent::TouchUpdate || event->type() == QEvent::TouchEnd))
