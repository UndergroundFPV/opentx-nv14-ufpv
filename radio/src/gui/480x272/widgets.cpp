...
void drawShutdownAnimation(uint32_t index, const char * message)
{
  ...
  if (shutdownBitmap && circleBitmap) {
    ...
    for (int i=1; i<=4; i++) {
      if (quarter == i) {
        lcd->drawMask(i <= 2 ?  LCD_W / 2 : (LCD_W - SHUTDOWN_CIRCLE_DIAMETER) / 2,
                      (i & 2) ? LCD_H / 2 : (LCD_H - SHUTDOWN_CIRCLE_DIAMETER) / 2,
                      circleBitmap, TEXT_COLOR, (SHUTDOWN_CIRCLE_DIAMETER / 2) * (i - 1), SHUTDOWN_CIRCLE_DIAMETER / 2);
      }
    }
  }
  else {
