  ...
  if (shutdown) {
    if (index < last_index) {
      ...
      if (shutdownCircle) {
        ...
        if (quarter >= 2) lcd->drawMask(LCD_W/2,                            LCD_H/2,                            shutdownCircle, TEXT_COLOR, SHUTDOWN_CIRCLE_DIAMETER/2, SHUTDOWN_CIRCLE_DIAMETER/2);
        if (quarter >= 3) lcd->drawMask((LCD_W-SHUTDOWN_CIRCLE_DIAMETER)/2, LCD_H/2,                            shutdownCircle, TEXT_COLOR, SHUTDOWN_CIRCLE_DIAMETER, SHUTDOWN_CIRCLE_DIAMETER/2);
        if (quarter >= 4) lcd->drawMask((LCD_W-SHUTDOWN_CIRCLE_DIAMETER)/2, (LCD_H-SHUTDOWN_CIRCLE_DIAMETER)/2, shutdownCircle, TEXT_COLOR, SHUTDOWN_CIRCLE_DIAMETER*3/2, SHUTDOWN_CIRCLE_DIAMETER/2);
      }
    }
  }
  else {
