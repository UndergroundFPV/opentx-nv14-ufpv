...
void intmoduleStop()
{
  ...
  NVIC_DisableIRQ(INTMODULE_TX_DMA_Stream_IRQnn);
...
void intmoduleSendNextFrame()
{
  if (s_current_protocol[INTERNAL_MODULE] == PROTO_PXX) {
    ...
    INTMODULE_TX_DMA_STREAM->CR &= ~DMA_SxCR_EN; // Disable DMA
    INTMODULE_TX_DMA_STREAM->CR |= INTMODULE_DMA_CHANNEL | DMA_SxCR_DIR_0 | DMA_SxCR_MINC | DMA_SxCR_PSIZE_0 | DMA_SxCR_MSIZE_0 | DMA_SxCR_PL_0 | DMA_SxCR_PL_1;
    INTMODULE_TX_DMA_STREAM->PAR = CONVERT_PTR_UINT(&INTMODULE_TIMER->ARR);
    INTMODULE_TX_DMA_STREAM->M0AR = CONVERT_PTR_UINT(modulePulsesData[INTERNAL_MODULE].pxx.pulses);
    INTMODULE_TX_DMA_STREAM->NDTR = modulePulsesData[INTERNAL_MODULE].pxx.ptr - modulePulsesData[INTERNAL_MODULE].pxx.pulses;
    INTMODULE_TX_DMA_STREAM->CR |= DMA_SxCR_EN | DMA_SxCR_TCIE; // Enable DMA
  }
    ...
    INTMODULE_TX_DMA_STREAM->CR |= INTMODULE_DMA_CHANNEL | DMA_SxCR_DIR_0 | DMA_SxCR_MINC | DMA_SxCR_PSIZE_0 | DMA_SxCR_MSIZE_0 | DMA_SxCR_PL_0 | DMA_SxCR_PL_1;
    INTMODULE_TX_DMA_STREAM->PAR = CONVERT_PTR_UINT(&INTMODULE_TIMER->ARR);
    INTMODULE_TX_DMA_STREAM->M0AR = CONVERT_PTR_UINT(modulePulsesData[INTERNAL_MODULE].ppm.pulses);
    INTMODULE_TX_DMA_STREAM->NDTR = modulePulsesData[INTERNAL_MODULE].ppm.ptr - modulePulsesData[INTERNAL_MODULE].ppm.pulses;
    INTMODULE_TX_DMA_STREAM->CR |= DMA_SxCR_EN | DMA_SxCR_TCIE; // Enable DMA
  }
#endif
  else {
...
void intmodulePxxStart()
{
  ...
  NVIC_EnableIRQ(INTMODULE_TX_DMA_Stream_IRQnn);
  NVIC_SetPriority(INTMODULE_TX_DMA_Stream_IRQnn, 7);
  NVIC_EnableIRQ(INTMODULE_TIMER_CC_IRQn);
  NVIC_SetPriority(INTMODULE_TIMER_CC_IRQn, 7);
}
