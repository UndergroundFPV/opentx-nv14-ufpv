...
//! djb2 hash algorithm
static inline const uint32_t hash(const void * ptr, uint32_t size)
{
...
static inline const uint16_t isqrt32(uint32_t n)
{
  uint16_t c = 0x8000;
  uint16_t g = 0x8000;
...
static inline const float otx_sinf(float x)
{
#if defined(CPUARM) && defined(__GNUC__)
...
static inline const float otx_cosf(float x)
{
#if defined(CPUARM) && defined(__GNUC__)
...
static inline const void otx_sincosf(float x, float * s, float * c)
{
#if defined(CPUARM) && defined(__GNUC__)
